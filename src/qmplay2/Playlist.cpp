...
Playlist *Playlist::create(const QString &url, OpenMode openMode, QString *name)
{
    ...
    if (openMode == ReadOnly)
    {
        for (Module *module : QMPlay2Core.getPluginsInstance())
        {
            for (const Module::Info &mod : module->getModulesInfo())
            {
                if (mod.type != Module::PLAYLIST || !mod.extensions.contains("m3u"))
                    continue;
                ...
                IOController<Reader> &reader = playlist->ioCtrl.toRef<Reader>();
                if (Reader::create(url, reader) && reader)
                {
                    const QByteArray header = reader->read(16);
                    reader->seek(0);

                    const QByteArray trimmed = header.trimmed();

                    if (trimmed.startsWith("#EXTM3U") ||
                        trimmed.startsWith("\xEF\xBB\xBF#EXTM3U"))
                    {
                        if (name)
                            *name = mod.name;
                        return playlist;
