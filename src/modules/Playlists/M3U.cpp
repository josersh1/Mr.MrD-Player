...
Playlist::Entries M3U::read()
{
    ...
    for (const QByteArray &line : readLines())
    {
        ...
        else if (!line.startsWith("#"))
        {
            ...
            const int logoStart = extinf[0].indexOf("tvg-logo=\"");
            
            // DEBUG TVG LOGO
            // qDebug() << "EXTINF ATTRS:" << extinf[0];
            // qDebug() << "LOGO START:" << logoStart;
            if (logoStart >= 0)
            {
                const int valueStart = logoStart + 10;
                const int valueEnd = extinf[0].indexOf('"', valueStart);
                if (valueEnd > valueStart)
                    entry.params[Entry::TvgLogoParam] = extinf[0].mid(valueStart, valueEnd - valueStart).toUtf8();
            }
            ...
            // TEMP DEBUG
            // qDebug() << "EXTVLCOPT keys:" << extvlcopt.keys();
            
            // ERRORES HEADERS
            // extvlcopt.keys();
            
            // NEW ENTRY FOR REFERER COMPATIBILITY
            auto referrer = extvlcopt.value("http-referrer");
            if (referrer.isEmpty())
                referrer = extvlcopt.value("http-referer");
            if (!referrer.isEmpty())
