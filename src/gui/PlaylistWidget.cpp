    ...
    repaintAll(false)
{
    ...
    connect(&logoNetwork, &NetworkAccess::finished, this, &PlaylistWidget::logoDownloadFinished);
}
...
QTreeWidgetItem *PlaylistWidget::newEntry(const Playlist::Entry &entry, QTreeWidgetItem *parent, const Functions::DemuxersInfo &demuxersInfo, int insertChildAt, QStringList *existingEntries)
{
    ...
    QMetaObject::invokeMethod(this, [this, tWI, icon, parent, insertChildAt, logoUrl = entry.params.value(Playlist::Entry::TvgLogoParam)] {
        setEntryIcon(icon, tWI);
        insertItem(tWI, parent, insertChildAt);

        if (!logoUrl.isEmpty())
        {
            if (NetworkReply *reply = logoNetwork.start(QString::fromUtf8(logoUrl)))
                logoReplies.insert(reply, tWI);
        }
    });

    return tWI;
}
void PlaylistWidget::logoDownloadFinished(NetworkReply *reply)
{
    const auto it = logoReplies.find(reply);
    if (it == logoReplies.end())
        return;

    QTreeWidgetItem *tWI = it.value();
    logoReplies.erase(it);
